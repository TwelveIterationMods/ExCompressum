...
public class CompressedEnemyHandler {
    ...
    // TODO need join world and death event in Balm
    /*public static void initialize() {
        Balm.getEvents().onEvent(EntityJoinWorldEvent.class, CompressedEnemyHandler::onSpawnEntity);
        Balm.getEvents().onEvent(LivingDeathEvent.class, CompressedEnemyHandler::onEntityDeath);
    }

    public static void onSpawnEntity(EntityJoinWorldEvent event) {
        if (!event.getLevel().isClientSide && (event.getEntity() instanceof Mob || event.getEntity() instanceof Ghast)) {
            ...
            if (registryName != null && ExCompressumConfig.getActive().compressedMobs.compressedMobAllowedMobs.contains(registryName.toString()) == isWhitelist) {
                ...
                if (event.getEntity().level.random.nextFloat() < ExCompressumConfig.getActive().compressedMobs.compressedMobChance) {
                    ...
                    event.getEntity().setCustomName(Component.translatable("excompressum.compressedMob", event.getEntity().getName()));

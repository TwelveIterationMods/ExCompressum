...
public class SieveMeshRegistry {
    ...
    private static final Map<Identifier, SieveMeshRegistryEntry> entriesByItem = new HashMap<>();

    public static void registerDefaults(@Nullable Object backingMesh) {
        SieveMeshRegistryEntry ironMesh = new SieveMeshRegistryEntry(CommonMeshType.IRON, ModItems.ironMesh.createStack(), backingMesh);
        ironMesh.setHeavy(true);
        ironMesh.setModelName("iron");
        add(ironMesh);
    }

    public static Map<Identifier, SieveMeshRegistryEntry> getEntries() {
    ...
    public static void add(SieveMeshRegistryEntry sieveMesh) {
        ...
        final var itemId = BuiltInRegistries.ITEM.getKey(sieveMesh.getItemStack().getItem());

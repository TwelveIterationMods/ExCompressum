...
public class ModTextures {
    ...
    public static void afterTextureStitch(TextureStitchEvent.Post event) {
        if (event.getAtlas().location().equals(InventoryMenu.BLOCK_ATLAS)) {
            iconEmptyBookSlot = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "items/empty_enchanted_book_slot"));
            iconEmptyMeshSlot = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "items/empty_mesh_slot"));
            iconEmptyHammerSlot = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "items/empty_hammer_slot"));
            iconEmptyCompressedHammerSlot = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "items/empty_compressed_hammer_slot"));
            stringMeshSprite = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "blocks/string_mesh"));
            flintMeshSprite = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "blocks/flint_mesh"));
            ironMeshSprite = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "blocks/iron_mesh"));
            diamondMeshSprite = event.getAtlas().getSprite(new ResourceLocation(ExCompressum.MOD_ID, "blocks/diamond_mesh"));

            for (int i = 0; i < destroyBlockIcons.length; i++) {
                destroyBlockIcons[i] = event.getAtlas().getSprite(new ResourceLocation("blocks/destroy_stage_" + i));
            }
        }
    }
}

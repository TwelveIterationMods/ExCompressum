...
public abstract class BlockAutoSieveBase extends BlockContainer {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int side, float hitX, float hitY, float hitZ) {
		...
		if (heldItem != null) {
			TileEntityAutoSieveBase tileEntity = (TileEntityAutoSieveBase) world.getTileEntity(x, y, z);
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int metadata) {
		...
		if (((TileEntityAutoSieveBase) tileEntity).getCurrentStack() != null) {
			EntityItem entityItem = new EntityItem(world, x, y, z, ((TileEntityAutoSieveBase) tileEntity).getCurrentStack());
			double motion = 0.05;
			entityItem.motionX = world.rand.nextGaussian() * motion;
			entityItem.motionY = 0.2;
			entityItem.motionZ = world.rand.nextGaussian() * motion;

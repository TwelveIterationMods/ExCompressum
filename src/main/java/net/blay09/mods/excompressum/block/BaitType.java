...
public enum BaitType implements IStringSerializable { // TODO shouldn't be an enum, registrify
    ...
    private List<BaitTileEntity.BaitBlockCondition> environmentConditions;
    ...
    public Collection<BaitTileEntity.BaitBlockCondition> getEnvironmentConditions() {
        if (environmentConditions == null) {
            if (this == OCELOT || this == PARROT) {
                environmentConditions = Lists.newArrayList(
                        new BaitTileEntity.BaitBlockCondition(Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, BlockPlanks.EnumType.JUNGLE), false),
                        new BaitTileEntity.BaitBlockCondition(Blocks.VINE.getDefaultState(), true),
                        new BaitTileEntity.BaitBlockCondition(Blocks.WATERLILY.getDefaultState(), false),
                        new BaitTileEntity.BaitBlockCondition(Blocks.SAPLING.getDefaultState().withProperty(BlockSapling.TYPE, BlockPlanks.EnumType.JUNGLE), false)
                );
            } else if (this == SQUID) {
                environmentConditions = Lists.newArrayList(
                        new BaitTileEntity.BaitBlockCondition(Blocks.WATER.getDefaultState(), true),
                        new BaitTileEntity.BaitBlockCondition(Blocks.FLOWING_WATER.getDefaultState(), true)
                );
            } else if (this == POLAR_BEAR) {
                environmentConditions = Lists.newArrayList(
                        new BaitTileEntity.BaitBlockCondition(Blocks.WATER.getDefaultState(), true),
                        new BaitTileEntity.BaitBlockCondition(Blocks.FLOWING_WATER.getDefaultState(), true),
                        new BaitTileEntity.BaitBlockCondition(Blocks.SNOW.getDefaultState(), true),
                        new BaitTileEntity.BaitBlockCondition(Blocks.WATER.getDefaultState(), true),
                        new BaitTileEntity.BaitBlockCondition(Blocks.SNOW_LAYER.getDefaultState(), true)
                );
            } else {
                ...
                environmentConditions = Lists.newArrayList(
                        new BaitTileEntity.BaitBlockCondition(Blocks.GRASS.getDefaultState(), false),
                        new BaitTileEntity.BaitBlockCondition(Blocks.TALLGRASS.getDefaultState().withProperty(BlockTallGrass.TYPE, BlockTallGrass.EnumType.GRASS), false),
                        new BaitTileEntity.BaitBlockCondition(Blocks.TALLGRASS.getDefaultState().withProperty(BlockTallGrass.TYPE, BlockTallGrass.EnumType.FERN), false)
                );
                for (BlockPlanks.EnumType treeType : acceptedTrees) {
                    environmentConditions.add(new BaitTileEntity.BaitBlockCondition(Blocks.SAPLING.getDefaultState().withProperty(BlockSapling.TYPE, treeType), false));
                    if (BlockOldLog.VARIANT.getAllowedValues().contains(treeType)) {
                        environmentConditions.add(new BaitTileEntity.BaitBlockCondition(Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, treeType), false));
                    }
                    if (BlockNewLog.VARIANT.getAllowedValues().contains(treeType)) {
                        environmentConditions.add(new BaitTileEntity.BaitBlockCondition(Blocks.LOG2.getDefaultState().withProperty(BlockNewLog.VARIANT, treeType), false));
                    }
                }
            }
        }

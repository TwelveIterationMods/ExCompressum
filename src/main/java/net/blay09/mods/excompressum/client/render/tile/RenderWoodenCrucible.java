...
public class RenderWoodenCrucible extends TileEntitySpecialRenderer<TileWoodenCrucible> {
	...
	public void renderTileEntityAt(TileWoodenCrucible tileEntity, double x, double y, double z, float partialTicks, int destroyStage) {
		...
		if (outputStack != null) {
			...
			GlStateManager.scale(0.875f, 0.75f, 0.875f);
		...
		if (solidVolume > 0) {
			...
			GlStateManager.scale(0.875f, 0.71 * (float) solidVolume / (float) tileEntity.getSolidCapacity(), 0.875f);
			RenderUtils.renderBlockWithTranslate(mc, Blocks.LEAVES.getDefaultState(), tileEntity.getWorld(), tileEntity.getPos(), renderer);
			tessellator.draw();
			GlStateManager.popMatrix();
		}
